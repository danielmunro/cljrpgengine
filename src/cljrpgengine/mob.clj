        ...
        is-blocked? (fn [direction to-x to-y]
                      (let [rx (Math/round ^float to-x)
                        ...
                        (when (= :right direction)
                          (swap! cells conj [(inc (Math/floor to-x)) ry])
                          (if (< ry to-y)
                            (swap! cells conj [(inc (Math/floor to-x)) (inc (Math/floor to-y))])
                          (if (< to-y ry)
